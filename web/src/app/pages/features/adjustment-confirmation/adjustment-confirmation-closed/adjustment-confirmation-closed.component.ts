...
@Component({
  selector: 'app-adjustment-confirmation-closed',
  templateUrl: './adjustment-confirmation-closed.component.html',
  styleUrls: ['./adjustment-confirmation-closed.component.scss'],
  host: {
    class: "page-component"
  }
})
export class AdjustmentConfirmationClosedComponent {
  tabIndex = 0;
  ...
  async getInventoryAdjustmentReportPaginated(table: "closed" | "rejected"){
    try{
        ...
        if(res.success){
          let data = res.data.results.map((d)=>{
            return {
              ...
              url: `/adjustment-confirmation/closed-report/${d.inventoryAdjustmentReportCode}/details`,
  ...
  onSelectedTabChange({ index }, redirect = true) {
    if(index === 1) {
      if(redirect) {
        this._location.go("/adjustment-confirmation/closed-report/rejected");
      }
      this.titleService.setTitle(`Rejected | ${this.appConfig.config.appName}`);
    } else {
      if(redirect) {
        this._location.go("/adjustment-confirmation/closed-report/closed");
      }
