...
import { AdjustmentConfirmationTableColumn, InventoryAdjustmentReportTableColumn } from 'src/app/shared/utility/table';
import { Users } from 'src/app/model/users';
import { Title } from '@angular/platform-browser';
import { Location } from '@angular/common';

@Component({
  selector: 'app-adjustment-confirmation',
  templateUrl: './adjustment-confirmation.component.html',
  styleUrls: ['./adjustment-confirmation.component.scss'],
  host: {
    class: "page-component"
  }
})
export class AdjustmentConfirmationComponent {
  tabIndex = 0;
  ...
  async getInventoryAdjustmentReportPaginated(table: "pending" | "reviewed" | "completed"){
    try{
      let findIndex = this.filter[table].findIndex(x=>x.apiNotation === "reportStatus");
      if(findIndex >= 0) {
        ...
        if(res.success){
          let data = res.data.results.map((d)=>{
            return {
              ...
              url: `/adjustment-confirmation/${d.inventoryAdjustmentReportCode}/details`,
            } as AdjustmentConfirmationTableColumn
          });
  ...
  onSelectedTabChange({ index }, redirect = true) {
    if(index === 1) {
      if(redirect) {
        this._location.go("/adjustment-confirmation/reviewed");
      }
      this.titleService.setTitle(`Reviewed | ${this.appConfig.config.appName}`);
    } else if(index === 2) {
      if(redirect) {
        this._location.go("/adjustment-confirmation/completed");
      }
      this.titleService.setTitle(`Completed | ${this.appConfig.config.appName}`);
    } else {
      if(redirect) {
        this._location.go("/adjustment-confirmation/pending");
      }
