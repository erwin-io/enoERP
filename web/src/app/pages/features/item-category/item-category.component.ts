...
import { ItemCategoryTableColumn } from 'src/app/shared/utility/table';
...
export class ItemCategoryComponent {
  ...
  @ViewChild('itemCategoryFormDialog') itemCategoryFormDialogTemp: TemplateRef<any>;
  constructor(
  ...
  async pageChange(event: { pageIndex: number, pageSize: number }) {
    ...
    await this.getItemCategoryPaginated();
  }

  async sortChange(event: { active: string, direction: string }) {
    const { active, direction } = event;
    const { apiNotation } = this.appConfig.config.tableColumns.itemCategory.find(x=>x.name === active);
  ...
  showAddDialog() {
    this.dialog.open(this.itemCategoryFormDialogTemp)
  }

  closeNewItemCategoryDialog() {
    this.dialog.closeAll();
  }

  saveNewItemCategory(formData) {
    ...
    dialogRef.componentInstance.conFirm.subscribe(async (data: any) => {
      ...
      try {
        let res = await this.itemCategoryService.create(formData).toPromise();
        if (res.success) {
