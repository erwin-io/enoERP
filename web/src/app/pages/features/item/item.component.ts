...
import { ItemTableColumn } from 'src/app/shared/utility/table';
...
export class ItemComponent {
  ...
  @ViewChild('itemFormDialog') itemFormDialogTemp: TemplateRef<any>;
  constructor(
  ...
  async pageChange(event: { pageIndex: number, pageSize: number }) {
    ...
    await this.getItemPaginated();
  }

  async sortChange(event: { active: string, direction: string }) {
    const { active, direction } = event;
    const { apiNotation } = this.appConfig.config.tableColumns.item.find(x=>x.name === active);
        ...
        if(res.success){
          let data = res.data.results.map((d)=>{
            return {
              ...
              price: d.price,
              itemCategory: d.itemCategory?.name,
              url: `/item/${d.itemId}`,
            } as ItemTableColumn
          });
  ...
  showAddDialog() {
    this.dialog.open(this.itemFormDialogTemp)
  }
  ...
  saveNewItem(formData) {
    ...
    dialogRef.componentInstance.conFirm.subscribe(async (data: any) => {
      ...
      try {
        let res = await this.itemService.create(formData).toPromise();
        if (res.success) {
