...
export class WarehouseDetailsComponent {
  ...
  @ViewChild('warehouseForm', { static: true}) warehouseForm: WarehouseFormComponent;
  ...
  constructor(
    ...
    this.isNew = isNew;
    this.id = this.route.snapshot.paramMap.get('warehouseId');
  ...
  get pageRights() {
    let rights = {};
    // for(var right of this.pageWarehouse.rights) {
    //   rights[right] = this.pageWarehouse.modify;
    // }
    return rights;
  }
  ...
  onDelete() {
    ...
    dialogData.message = 'Delete warehouse?';
    ...
    dialogRef.componentInstance.conFirm.subscribe(async (data: any) => {
      ...
      try {
        let res = await this.warehouseService.delete(this.id).toPromise();
        if (res.success) {
  ...
  onUpdate(formData) {
    ...
    dialogRef.componentInstance.conFirm.subscribe(async (data: any) => {
      ...
      dialogRef.componentInstance.isProcessing = this.isProcessing;
      try {
        let res = await this.warehouseService.update(this.id, formData).toPromise();
        if (res.success) {
          this.snackBar.open('Saved!', 'close', {
            panelClass: ['style-success'],
          });
