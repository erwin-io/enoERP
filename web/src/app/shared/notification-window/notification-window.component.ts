...
export class NotificationWindowComponent {
  ...
  async onNotificationItemClick(notification: Notifications) {
    ...
    this.readNotif.emit();
    if(notification.type === "GOODS_RECEIPT") {
      window.open("/goods-receipt/" + notification.referenceId + "/details");
    }
    else if(notification.type === "GOODS_ISSUE") {
      window.open("/goods-issue/" + notification.referenceId + "/details");
    }
    else if(notification.type === "INVENTORY_REQUEST") {
      if(this.profile.branch.isMainBranch) {
        window.open("/incoming-inventory-request/" + notification.referenceId + "/details");
      } else {
        window.open("/inventory-request/" + notification.referenceId + "/details");
      }
    }
